...
import { Loader2, Play, Lock, Dumbbell, Flag } from "lucide-react";
  ...
  if (chaptersError) {
    return (
      <div className="border-4 border-gray-800 rounded-lg p-4 sm:p-6 lg:p-8 bg-red-50 dark:bg-red-900/20 text-center mx-4">
        <h3 className="text-xl sm:text-2xl font-game font-normal mb-4 text-red-800 dark:text-red-200">
  ...
  if (chapters.length === 0) {
    return (
      <div className="border-4 border-gray-800 rounded-lg p-4 sm:p-6 lg:p-8 bg-white dark:bg-gray-800 text-center mx-4">
        <h3 className="text-xl sm:text-2xl font-game font-normal mb-4">
    ...
    <>
      <div id="course-chapters" className="space-y-4 sm:space-y-6 ">
        <div className="rounded-lg bg-white dark:bg-gray-800 p-4 sm:p-6 border-4 border-gray-800 shadow-[6px_6px_0_0_#000] dark:shadow-[6px_6px_0_0_#fff] max-h-210 overflow-y-auto">
          <h2 className="text-2xl sm:text-3xl font-game font-normal mb-4 sm:mb-6">
            Course Chapters
          </h2>
          ...
          >
            {chapters.map((chapter, index) => {
              ...
              return (
                ...
                >
                  <AccordionTrigger className="px-3 sm:px-6 py-3 sm:py-4 hover:no-underline hover:bg-gray-100 dark:hover:bg-gray-700 transition-all font-game font-normal text-base sm:text-lg text-left hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-[2px_2px_0_0_#000] dark:hover:shadow-[2px_2px_0_0_#fff]">
                    <div className="flex items-center gap-2 sm:gap-4 w-full">
                      ...
                      <span className="flex-1 truncate text-2xl font-normal text-black dark:text-white">
                        {chapter.name}
                      </span>
                      <span
                        className={`text-[10px] sm:text-xs font-mono border px-1 sm:px-2 py-0.5 rounded-none shrink-0 ${statusStyles[status]}`}
                      >
                        {statusLabel}
                      </span>
                    </div>
                    ...
                    <div className="space-y-3 sm:space-y-4 pt-3 sm:pt-4 border-t-4 border-gray-800 border-dashed">
                      ...
                      {chapter.exercise && (
                        <div className="bg-yellow-100 dark:bg-yellow-900/30 border-4 border-yellow-500 rounded-none p-3 sm:p-4 shadow-[4px_4px_0_0_#d97706]">
                          <h4 className="font-game font-normal text-yellow-800 dark:text-yellow-200 mb-2 text-base sm:text-lg flex items-center gap-2">
                        ...
                        <p className="text-xs font-mono flex items-center gap-2">
                          {isEnrolled ? (
                            completedCount >= totalChapters ? (
                              <>
                                <Flag className="w-4 h-4" /> You finished every
                              ...
                              <>
                                ...
                                />
                                {totalChapters - completedCount} chapters left.
                                Keep going!{" "}
                              </>
                            )
                          ) : (
